...
echo $listaCampos[1];
   for($i = 0; $i<4; $i++ ){
		$var = "{$var} {$listaCampos[$i]} = '{$listaValores[$i]}' ";
   ...
   //----------------------------------------------------------------------------------------------------
    $insertSQL = "UPDATE $tabla"." SET ".trim($var, ',')." WHERE  $listaCampos[0] = $listaValores[0];";
	echo "<h3>SQL resultante:</h3>" . $insertSQL;
	
	//"UPDATE ".$tabla. " SET (" . $var . ") WHERE  (" . $listaCampos[0] . ") = (" . $listaValores[0] . ");";
   ...
   //UPDATE tabla SET WHERE ID = ?
    ...
    $conexion_info = mysqli_connect("127.0.0.1:3307",'root','',$base);  /* conexion a BD  */
